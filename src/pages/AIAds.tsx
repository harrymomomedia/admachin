...
export function AIAds() {
    ...
    const [data, setData] = useState<AIGeneratedAd[]>([]);
    ...
    const renderPromptsButton = (prompts: AIPrompts | null) => {
        ...
        return (
            <button
                onClick={() => setPromptModal({ isOpen: true, title: 'Ad Copy Generation', prompts })}
    ...
    const columns: ColumnDef<AIGeneratedAd>[] = [
        createIdColumn<AIGeneratedAd>(),
        ...
        createSubprojectColumn<AIGeneratedAd>({ projects, subprojects, subprojectColorMap }),
        {
            ...
            width: 80,
            type: 'custom',
            render: (_value: unknown, row: AIGeneratedAd) => renderPromptsButton(row.prompts)
        },
        createUserColumn<AIGeneratedAd>(users, { key: 'created_by', editable: false }),
    ...
    const handleCreateRow = async (defaults?: Record<string, unknown>): Promise<AIGeneratedAd> => {
        const newRow = await createAIGeneratedAd({
            content: 'New Ad Copy',
            ad_type: 'FB Ad Text',
    ...
    return (
        <DataTablePageLayout>
            <DataTable
                ...
                isLoading={isLoading}
                title="Generated Ads"
                emptyMessage="No generated ads saved yet. Click + to create one!"
                getRowId={(row) => row.id}
