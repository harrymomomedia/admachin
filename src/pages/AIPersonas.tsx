...
export function AIPersonas() {
    ...
    const [data, setData] = useState<AIPersona[]>([]);
    ...
    const renderPromptsButton = (prompts: AIPrompts | null) => {
        ...
        return (
            <button
                onClick={() => setPromptModal({ isOpen: true, title: 'Persona Generation', prompts })}
    ...
    const columns: ColumnDef<AIPersona>[] = [
        createIdColumn<AIPersona>(),
        { key: 'content', header: 'Content', editable: true, type: 'longtext', width: 500 },
        createProjectColumn<AIPersona>({ projects, subprojects, projectColorMap }),
        createSubprojectColumn<AIPersona>({ projects, subprojects, subprojectColorMap }),
        { key: 'rich_text', header: 'Rich Text', editable: true, type: 'blocknoteeditor', width: 300, minWidth: 200 },
        {
            ...
            width: 80,
            type: 'custom',
            render: (_value: unknown, row: AIPersona) => renderPromptsButton(row.prompts)
        },
        createUserColumn<AIPersona>(users, { key: 'created_by', editable: false }),
        { key: 'created_at', header: 'Created', type: 'date', width: 120 },
    ];

    const handleUpdate = useMemo(() => createUpdateHandler<AIPersona>({
        updateFn: updateAIPersona,
        setData,
    }), []);

    const handleDelete = useMemo(() => createDeleteHandler<AIPersona>({
        deleteFn: deleteAIPersona,
        setData,
        confirmMessage: false, // No confirmation for personas
    }), []);
    ...
    return (
        <DataTablePageLayout>
            <DataTable
                ...
                isLoading={isLoading}
                title="Personas"
                emptyMessage="No personas saved yet. Click + to create one!"
                getRowId={(row) => row.id}
